...
void gameBcgInit()
{
    ...
    {
        SpriteClassId scid = spritesGetIdByNameF("bcg-layer-%d", i);
        
        int width;
        int height;

        spritesGetDimensions(scid, &width, &height);
        ...
        for(j = 0; j < count; ++j)
        {
            sprites[j] = sngeAddSprite(scid, point(xpos, (j - 1) * height), i);
